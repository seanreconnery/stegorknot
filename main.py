...
def embed():
	...
	if file and allowed_file(file.filename):
		...
		pdf = s.find(b"\x25\x50\x44\x46\x2d")
		lzip = s.find(b"\x4C\x5A\x49\x50")
		pkzip = s.find(b"\x50\x4B\x03\x04")
		utf8 = s.find(b"\xEF\xBB\xBF")
		tar = s.find(b"\x75\x73\x74\x61\x72")
		xzlz = s.find(b"\xFD\x37\x7A\x58\x5A\x00\x00")
		doc = s.find(b"\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1")
		...
		resp2 = jsonify({'error': 'Allowed file types are jpg or jpeg'})
		resp2.status_code = 400
		return resp2
